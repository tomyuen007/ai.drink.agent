#!/usr/bin/env bash
# Weather AI startup — CPU-only (no GPU required)
# Covers: venv creation, PyTorch CPU install, all Python packages, npm, agent + Expo
#
# Usage:
#   bash scripts/start-weather-nogpu.sh              # setup + start
#   bash scripts/start-weather-nogpu.sh --setup-only # setup only, do not start servers
...
echo -e "\n${BOLD}Weather AI — CPU-only startup${NC}"
...
# ── 3. PyTorch — CPU-only (must come before requirements.txt) ──────────────────
section "PyTorch (CPU-only)"

if python -c "import torch" 2>/dev/null; then
  TORCH_VER=$(python -c "import torch; print(torch.__version__)")
  CUDA_AVAIL=$(python -c "import torch; print(torch.cuda.is_available())")
  if [[ "$CUDA_AVAIL" == "True" ]]; then
    warn "torch $TORCH_VER is installed with CUDA support — reinstalling CPU build"
    pip install --quiet torch --index-url https://download.pytorch.org/whl/cpu
  else
    info "torch $TORCH_VER (CPU) already installed"
  fi
else
  info "Installing torch CPU build (~200 MB)..."
...
if [[ "$SETUP_ONLY" == true ]]; then
  ...
  echo "To start the system, run: bash scripts/start-weather-nogpu.sh"
  exit 0
fi
